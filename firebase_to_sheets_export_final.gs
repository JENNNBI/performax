...
function getFirestoreDataToSheet() {
  ...
  // FIX: Snapshot yapısını kontrol et ve debug bilgisi yazdır
  Logger.log('Snapshot tipi: ' + typeof snapshot);
  Logger.log('Snapshot içeriği: ' + JSON.stringify(snapshot));
  ...
  // FIX: Farklı olası yapıları kontrol et
  let documents = null;
  
  // Olasılık 1: snapshot.documents (en yaygın)
  ...
  // Olasılık 2: snapshot doğrudan bir dizi
  else if (Array.isArray(snapshot)) {
    documents = snapshot;
    Logger.log('Snapshot doğrudan bir dizi: ' + documents.length + ' adet');
  }
  // Olasılık 3: snapshot içinde başka bir yapı
  else if (typeof snapshot === 'object') {
    // Tüm özellikleri kontrol et
    Logger.log('Snapshot özellikleri: ' + Object.keys(snapshot).join(', '));
    
    // Belge içeren ilk dizi özelliğini bul
  ...
  // Belgeler bulunamadıysa hata ver
  if (!documents || !Array.isArray(documents)) {
    Logger.log('HATA: Belgeler bulunamadı. Snapshot yapısı: ' + JSON.stringify(Object.keys(snapshot || {})));
    sheet.clear();
    sheet.appendRow(['HATA: Belgeler bulunamadı. Lütfen snapshot yapısını kontrol edin.']);
    sheet.appendRow(['Snapshot tipi: ' + typeof snapshot]);
    sheet.appendRow(['Snapshot özellikleri: ' + (snapshot ? Object.keys(snapshot).join(', ') : 'snapshot null/undefined')]);
    return;
  }
  ...
  documents.forEach(doc => {
    // FIX: doc ve doc.fields kontrolü ekle
    if (!doc || !doc.fields) {
      Logger.log('UYARI: Geçersiz belge atlandı.');
      return; // Bu belgeyi atla
    }

    const row = [];
    // FIX: doc.name kontrolü ekle
