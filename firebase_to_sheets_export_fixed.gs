...
function getFirestoreDataToSheet() {
  ...
  // FIX: Null/undefined kontrolü ekle
  if (!snapshot) {
  ...
  // FIX: documents özelliğinin varlığını kontrol et
  if (!snapshot.documents) {
    Logger.log('HATA: Snapshot.documents tanımlı değil.');
    sheet.clear();
    sheet.appendRow(['HATA: Snapshot.documents tanımlı değil.']);
    return;
  }

  const documents = snapshot.documents;

  // FIX: documents'in bir dizi olduğunu kontrol et
  if (!Array.isArray(documents)) {
    Logger.log('HATA: documents bir dizi değil. Tip: ' + typeof documents);
    sheet.clear();
    sheet.appendRow(['HATA: documents bir dizi değil.']);
    return;
  }
